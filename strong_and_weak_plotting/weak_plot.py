...
for ps in problem_sizes:
    ...
    plt.errorbar(threads, speedup, yerr=speedup_err, fmt='-o', capsize=5, label=f'base size = {ps}')

# Set plot properties
plt.xlabel('Number of Threads')
plt.ylabel('Scaled Speedup')
plt.title(f'Scaled Speedup by Number of Threads and Problem Size averaged over {num_reps} repetitions')
plt.legend()
plt.grid(True)
