...
fit_gustafson = sys.argv[6]
plot_ideal = sys.argv[7]

# Load the data
data = pd.read_csv('time.csv', header=None)
data.columns = ['problem_size', 'number_of_threads', 'execution_time']
# Multpily the execution time by the number of threads to get the total time
data['execution_time'] /= data['number_of_threads']
...
for i, ps in enumerate(problem_sizes):
    ...
    if fit_gustafson != '0':
        # fit a function of the form: y = s + (1 - s) * threads
        # to the speedup data
        def func(x, s):
            return s + (1 - s) * x
...
plt.title(f'Weak scaling for different problem sizes averaged over {num_reps} repetitions')
plt.legend()
plt.grid(True)

# Save the plot as a pdf
current_datetime = datetime.datetime.now().strftime("%m-%d_%H-%M:%S")
filename = f'weak_scaling_plot_{current_datetime}.pdf'
